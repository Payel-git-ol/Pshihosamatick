...
fun CheckEmailField(
    ...
    OutlinedTextField(
        ...
        label = {
            Text(
                text = if (isEmailValid) "Email" else "Invalid email",
                style = MaterialTheme.typography.labelMedium.copy(
                    fontSize = if (useIOSStyle) 15.sp else MaterialTheme.typography.labelMedium.fontSize
                ),
                color = if (!isEmailValid && email.isNotEmpty()) MaterialTheme.colorScheme.error
                else if (isFocused) {
        ...
        colors = OutlinedTextFieldDefaults.colors(
            ...
            errorBorderColor = if (useIOSStyle) Color(0xFFFF3B30) else MaterialTheme.colorScheme.error,
            errorLabelColor = if (useIOSStyle) Color(0xFFFF3B30) else MaterialTheme.colorScheme.error,
